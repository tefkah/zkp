...
import { IReactionGroups } from '@zkp/types'
import { GComment, GReactionGroup, GReply, GRepositoryDiscussion, GUser } from '@zkp/types'
...
export const adaptReactionGroups = (reactionGroups: GReactionGroup[]) =>
  reactionGroups.reduce((acc, group) => {
    acc[group.content] = {
...
export const adaptReply = (reply: GReply) => {
  const {
    reactionGroups,
...
export const adaptComment = (comment: GComment) => {
...
export const toggleEmail = (event: ReactMouseEvent<HTMLElement, MouseEvent>) => {
...
export const handleClipboardCopy = (event: ReactMouseEvent<HTMLElement, MouseEvent>) => {
  const element = event.target as HTMLElement
...
export const handleCommentClick = (event: ReactMouseEvent<HTMLElement, MouseEvent>) => {
  toggleEmail(event)
  handleClipboardCopy(event)
}

export const processCommentBody = (bodyHTML: string) => {
